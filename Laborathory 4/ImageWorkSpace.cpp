...
void Picture::OpenSingleFile(string filename)
{
    ...
    input.read((char*) &image[0].A, 1);
    for (int i = 0; i < width * height; i++)
    {
        input.read((char*) &image[i].A, 1);
        input.read((char*) &image[i].B, 1);
        input.read((char*) &image[i].C, 1);
    }
    if (!input.good())
...
void Picture::OpenMultipleFile(std::string filename)
{
    ...
    if (image == NULL)
        throw std::runtime_error("Out of memory");
    inputA.read((char*) &image[0].A, 1);
    inputB.read((char*) &image[0].A, 1);
    inputC.read((char*) &image[0].A, 1);
    for (int i = 0; i < width * height; i++)
    {
        inputA.read((char*) &image[i].A,1);
        inputB.read((char*) &image[i].B,1);
        inputC.read((char*) &image[i].C,1);
    }
}
