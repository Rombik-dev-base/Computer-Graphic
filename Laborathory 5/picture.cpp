//
// Created by 1285166 on 13.06.2020.
//
...
void Picture::OpenSingleFile(string filename)
{
    ...
    input >> maxvalue;
    if (!input.good())
        throw runtime_error("Invalid metadata");
    if (maxvalue != MAX_VALUE)
        throw runtime_error("Unacceptable format");
    char partPixel;
...
void Picture::SaveToSingleFile(string filename)
{
    ofstream output;
...
void Picture::autoBrightness(bool type) noexcept
{
    ...
    cout << BD[0] << ' ' << BD[1];
    double difference = image[BD[1]].A+image[BD[1]].B + image[BD[1]].C, multiple = 255.0/((double)BD[0] - (double)BD[1]);
    cout << "Difference: " << difference << ".\nMultiple: " << multiple << ".\n";
    changeBrightness(difference, multiple);
}

vector<int> Picture::getBrightestandDarkest(bool avoid) noexcept
{
